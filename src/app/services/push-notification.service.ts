...
export class PushNotificationService {
  private apiUrl = 'http://98.85.246.54:5966/api';
  ...
  sendNotification(data: { employeeId: string; token: string; title: string; body: string }) {
    const url = 'http://98.85.246.54:5966/api/notification/send-notification'; 
    return this.http.post<{ message: string }>(url, data);
  }
  
  getNotifications(employeeId: string): Observable<any> {
    return this.http.get(`http://98.85.246.54:5966/api/notification/get-notification/${employeeId}`);
  }
